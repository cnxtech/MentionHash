  ...
  template: `
    <div (mouseenter)="pause()" (mouseleave)="play()" class="carousel slide">
      ...
      <div style="float: right; margin: 20px">
        <button class="btn btn-primary" (click)="prev()" *ngIf="slides.length">Previous</button>
        <button class="btn btn-primary" (click)="next()" *ngIf="slides.length">Next</button>
      </div>

      <div class="dots" *ngIf="slides.length > 1">
        <span *ngFor="let slidez of slides" [ngClass]="(slidez.active === true)?'active':''"
              (click)="select(slidez)"></span>
      </div>

...
import "../../assets/jquery.timepicker.min.js";

@Directive({
  selector: '[timePicker][ngControl],[timePicker][ngFormControl],[timePicker][ngModel]',
  host: {
    '(change)': 'onChange($event.target.value)', '(blur)': 'onChange($event.target.value)'
  }
})
export class TimepickerDirective implements ControlValueAccessor, OnChanges {

  @Input()
  optionsTime: TimepickerConfig;
  ...
  ngOnChanges(): any {
    ...
    el.timepicker(this.optionsTime);
